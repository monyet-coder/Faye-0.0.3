...
//					'motorola'			=> 'Motorola'
    ...
    protected function setBrowser () {
        foreach((array)self::$browsers as $key => $val) {
    ...
    protected function setEngine () {
        foreach((array)self::$engines as $key => $val) {
            if(preg_match('|' . preg_quote($key) . '\/([0-9\.]+)|i', $this->agent, $match)) {
                $this->engine = $key;
                $this->series = $match[1];
    ...
    protected function setPlatform () {
        foreach((array)self::$platforms as $key => $val) {
	...
	{
        foreach ((array)self::$mobiles as $key => $val)
        {
